...
impl DiskImage {
    pub fn new_empty(media_type: u8) -> Result<Self, DiskError> {
        let (total_sectors, sectors_per_track, tracks, sides) = match media_type {
            ...
            _ => return Err(DiskError::FormatError(format!("Unsupported media type: 0x{:02X}", media_type))),
        };
    ...
    pub fn from_bytes(data: Vec<u8>) -> Result<Self, DiskError> {
        let (media_type, sectors_per_track, total_sectors, tracks, sides) = match data.len() {
            ...
            _ => return Err(DiskError::InvalidSize(format!(
        ...
        if data.len() >= SECTOR_SIZE {
            ...
            for sector in dir_start..dir_start + dir_sectors {
                ...
                if sector_offset + SECTOR_SIZE <= data.len() {
                    ...
                    for entry in 0..16 { // 512 / 32 = 16 entries per sector
                        ...
                        if entry_offset + 32 <= data.len() {
                            ...
                            if first_byte != 0x00 && first_byte != 0xE5 && first_byte != 0xFF {
                                ...
                                let filename: String = data[entry_offset..entry_offset + 11].iter()
    ...
    pub fn read_sectors(&self, start_sector: u16, count: u8) -> Result<Vec<u8>, DiskError> {
        ...
        let end_byte = start_byte + (count as usize * SECTOR_SIZE);

        let result = self.data[start_byte..end_byte].to_vec();
        
        // Log first few bytes when reading boot sector or directory sectors
        if start_sector == 0 || (start_sector >= 5 && start_sector <= 8) {
            let sector_name = if start_sector == 0 { "Boot" } else { "Directory" };
            tracing::trace!(
                "{} sector {} (bytes {}-{}): first 16 bytes = {:02X?}", 
                sector_name,
                start_sector,
                start_byte,
                end_byte,
    ...
    pub fn write_sectors(&mut self, start_sector: u16, data: &[u8]) -> Result<(), DiskError> {
        ...
        let start_byte = start_sector as usize * SECTOR_SIZE;
        self.data[start_byte..start_byte + data.len()].copy_from_slice(data);
    ...
    fn format_disk_data(data: &mut [u8], media_type: u8, total_sectors: u16) -> Result<(), DiskError> {
