...
impl WD2793 {
    pub fn new() -> Self {
        tracing::info!("[FDC] Initializing WD2793 FDC");
        Self {
            status_register: 0,
            command_register: 0,
            track_register: 0,
    ...
    pub fn read(&mut self, port: u8) -> u8 {
        tracing::info!("FDC: Read from port {:02X}", port);
        let result = match port & 0x03 {
            0 => {
                // Status register
                self.update_status();
                tracing::info!(
                    "FDC: Read status register: {:02X} (Motor:{}, Drive:{}, Disk:{}, Busy:{}, DRQ:{}, T0:{})", 
                    self.status_register,
                    if self.motor_on { "ON" } else { "OFF" },
                    self.current_drive,
                    if self.drives[self.current_drive as usize].is_some() { "YES" } else { "NO" },
                    self.busy,
                    self.drq,
                    self.track_zero
                );
                // Clear transient index pulse after reading
                if self.index_pulse {
            ...
            1 => {
                tracing::info!("FDC: Read track register: {}", self.track_register);
                self.track_register
            }
            2 => {
                tracing::info!("FDC: Read sector register: {}", self.sector_register);
    ...
    pub fn write(&mut self, port: u8, value: u8) {
        match port & 0x03 {
            0 => {
                // Command register
                tracing::info!("FDC: Command write {:02X} to port {:02X}", value, port);
    ...
    pub fn insert_disk(&mut self, drive: usize, image: DiskImage) {
        if drive < 2 {
            tracing::info!(
                "FDC: Inserting disk in drive {}, size: {} bytes",
                drive,
                image.data.len()
            );
            self.drives[drive] = Some(image);
        } else {
            tracing::warn!("FDC: Invalid drive number: {}", drive);
        }
    }
    ...
    fn execute_command(&mut self, cmd: u8) {
        ...
        tracing::info!(
            "FDC: Executing command {:02X} (type: {:X})",
            cmd,
            command_type
        );
    ...
    fn update_status(&mut self) {
        ...
        let drive_ready = self.motor_on
            && self.current_drive < 2
            && self.drives[self.current_drive as usize].is_some();
        ...
        tracing::debug!(
            "FDC: Status update - Motor: {}, Drive: {}, Has disk: {}, Status: {:02X}",
            self.motor_on,
            self.current_drive,
            self.drives[self.current_drive as usize].is_some(),
            self.status_register
